...
const CoachManagementScreen = () => {
  ...
  const renderCoachItem = ({ item: coach }) => (
    <View style={styles.coachCard}>
      <Image
        source={{ uri: coach.image || 'https://via.placeholder.com/150' }}
      ...
      <View style={styles.coachInfo}>
        ...
        <View style={styles.ratingContainer}>
          {coach.avgRating ? (
            <>
              <Text style={styles.ratingText}>Rating: {parseFloat(coach.avgRating).toFixed(2)}</Text>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            style={[
              styles.actionButton,
              { backgroundColor: getUserStatus(coach.userId) ? '#FF6347' : '#008080' },
            ]}
          ...
          >
            ...
            <Text style={styles.actionButtonText}>
              {getUserStatus(coach.userId) ? 'Deactivate' : 'Activate'}
            </Text>
          </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        <FlatList
          data={['All Sports', ...uniqueCoachingSports]}
          renderItem={({ item }) =>
            renderFilterItem({
              item,
              isSelected: item === 'All Sports' ? coachingSportFilter === '' : item === coachingSportFilter,
              onPress: () => handleFilter(item === 'All Sports' ? '' : item, coachLevelFilter),
            })
          }
      ...
      <View style={styles.filterContainer}>
        <FlatList
          data={['All Levels', ...uniqueCoachLevels]}
          renderItem={({ item }) =>
            renderFilterItem({
              item,
              isSelected: item === 'All Levels' ? coachLevelFilter === '' : item === coachLevelFilter,
              onPress: () => handleFilter(coachingSportFilter, item === 'All Levels' ? '' : item),
            })
          }
          keyExtractor={(item) => item}
          horizontal
