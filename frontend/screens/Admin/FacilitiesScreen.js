  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.actionContainer}>
        ...
        >
          <Ionicons
            name={facility.isActive ? 'power' : 'play'}
            size={16}
            color="#fff"
          />
...
const FacilityManagementScreen = () => {
  ...
  const toggleStatus = async (facilityId) => {
    try {
      const token = await AsyncStorage.getItem('userToken');
      const response = await axios.put(`${API_URL}/toggle/${facilityId}`, {}, {
        headers: {
          'x-auth-token': token,
        },
      });
  ...
  const handleSaveFacility = async (facilityData) => {
    try {
      ...
      if (selectedFacility) {
        setFacilities((prevFacilities) =>
          prevFacilities.map((facility) =>
            facility._id === savedFacility._id ? savedFacility : facility
          )
        );
      } else {
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              setFacilities(prevFacilities => 
                prevFacilities.filter(facility => facility._id !== facilityId)
              );
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => handleCategoryFilterChange(item)}
            >
              <Text style={[styles.filterButtonText, categoryFilter === item && styles.filterButtonTextActive]}>
                {item}
              </Text>
            </TouchableOpacity>
          )}
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={fetchFacilities} colors={['#008080']} />
        }
        ListEmptyComponent={
