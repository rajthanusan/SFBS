  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.actionContainer}>
        ...
        >
          <Ionicons name={equipment.isActive ? "power" : "play"} size={16} color="#fff" />
          <Text style={styles.actionButtonText}>{equipment.isActive ? "Deactivate" : "Activate"}</Text>
        </TouchableOpacity>
        <TouchableOpacity
...
const EquipmentModal = ({ isOpen, onClose, onSave, equipment }) => {
  ...
  const handleSave = () => {
    const equipmentData = {
      equipmentName,
      sportName,
      rentPrice: Number.parseFloat(rentPrice),
      image,
    }
    ...
    <Modal visible={isOpen} animationType="slide" transparent={true}>
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          <ScrollView>
            <Text style={styles.modalTitle}>{equipment ? "Edit Equipment" : "Add New Equipment"}</Text>
...
const EquipmentManagementScreen = () => {
  ...
  const handleSaveEquipment = async (equipmentData) => {
    try {
      ...
      if (selectedEquipment) {
        setEquipment((prevEquipment) =>
          prevEquipment.map((item) => (item._id === savedEquipment._id ? savedEquipment : item)),
        )
      } else {
  ...
  const handleDelete = async (equipmentId) => {
    Alert.alert("Confirm Deletion", "Are you sure you want to delete this equipment?", [
      {
        text: "Cancel",
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            ...
            setEquipment((prevEquipment) => prevEquipment.filter((item) => item._id !== equipmentId))
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        <View style={styles.dropdownRow}>
          ...
          />
          <SelectList
            setSelected={(val) => handleSportFilterChange(val)}
            data={SPORT_FILTERS.map((filter) => ({ key: filter, value: filter }))}
            save="value"
            search={false}
      ...
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={isLoading} onRefresh={fetchEquipment} colors={["#008080"]} />}
        ListEmptyComponent={
          <Text style={styles.emptyListText}>{isLoading ? "Loading equipment..." : "No equipment available"}</Text>
        }
      />
